...
```dart
void main() {
  RocketClient client = RocketClient(url: 'https://jsonplaceholder.typicode.com');
  Rocket.add('api', client);
  runApp(MyApp());
}
```
...
class PostList extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return RocketView(
      model: postModel,
      fetch: ({bool refresh = false}) => Rocket.get<RocketClient>('api').request('posts', model: postModel),
